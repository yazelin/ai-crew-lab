...
for (const c of CASES) {
  for (const [tag, sys] of [['舊(6 項)', OLD], ['新(7 項)', NEW]]) {
    ...
    const flagged = rows.filter((v) => v && /草稿|draft/.test(String(v.feedback || ''))).length;
    const reals = rows.filter(Boolean).map((v) => v.scores && v.scores.real);
    out[c.name + '｜' + tag] = { 通過: okN, 退件: parsed - okN, 抓草稿: flagged, real: reals };
    console.log('  ' + (c.name + '｜' + tag).padEnd(30)
      + '抓草稿 ' + flagged + '／' + N + '　通過 ' + okN + '　real ' + reals.join(','));
  }
}
